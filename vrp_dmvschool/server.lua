...
local choice_asklc = {function(player,choice)
  vRPclient.getNearestPlayer(player,{10},function(nplayer)
    ...
    if nuser_id ~= nil then
      vRPclient.notify(player,{"Intreaba de permis..."})
      vRP.request({nplayer,"Vrei sa iti arati permisul? ?",15,function(nplayer,ok)
        if ok then
          MySQL.query("vRP/dmv_search", {id = nuser_id}, function(rows, affected)
            if #rows > 0 then
			  vRPclient.notify(player,{"Permis: ~g~OK"})
			else
			  vRPclient.notify(player,{"Permis: ~r~Nu are"})
            end
          end)
        else
...
vRP.registerMenuBuilder({"police", function(add, data)
  ...
  if user_id ~= nil then
    ...
    if vRP.hasPermission({user_id,"police.askid"}) then
       choices["Verifica permis"] = choice_asklc
    end
